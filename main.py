try:
    ...
    window = Tk()
    window.withdraw()
    window.title("Contentkeeper Site thing - By Renderlabs & SchTools 2021")  # Mention this in the latest release too
    ...
    window.deiconify()
    Label(window, text="Website Thing", font=("Arial Bold", 25)).place(x=100, y=10)
    Label(window, text="Paste/type in the url you want to do stuff here (eg: google.com):").place(x=10, y=50)
    Label(window, text="Output:").place(x=10, y=125)

    url_input = Entry(window, width=79)
    url_input.place(x=10, y=75)

    btn = Button(window, text="Reclassify/IDK URL", command = lambda: reclassify(url_input.get()))
